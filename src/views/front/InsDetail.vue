<template>
  <div style="color: #666">
    <div style="margin: 20px 0; ">
      <div class="pd-10" style="font-size: 20px; color: #3F5EFB; cursor: pointer">{{ ins.insname }}</div>
      <div style="font-size: 14px; margin-top: 10px">
        <i class="el-icon-shopping-bag-1"></i> <span>￥{{ ins.insprice }}</span>
        <i class="el-icon-position" style="margin-left: 10px"></i> <span>{{ ins.address }}</span>
        <i class="el-icon-phone" style="margin-left: 10px"></i> <span>{{ ins.phone }}</span>
      </div>
    </div>
    ...
    <div style="margin: 30px 0">
      ...
      <div>
        <div v-for="item in comments" :key="item.id" style="border-bottom: 1px solid #ccc; padding: 10px 0; ">
          ...
          <div v-if="item.children != null"  style="padding-left: 200px">
            <div v-for="subItem in item.children" :key="subItem.id" style="padding: 0px 10px; background-color: #f0f0f0" >
              <!--              回复列表-->
              <div style=" font-size: 14px; padding: 5px 0; line-height: 25px">
                <div>
                  <b style="color: #F56C6C">@{{ subItem.pnickname }}：</b>
                </div>
...
export default {
  ...
  methods: {
    load(){
      this.request.get("/ins/" + this.id).then(res =>{
        console.log(res)
        this.ins = res.data
      })
    ...
    save(){
      ...
      this.commentForm.insId = this.id
